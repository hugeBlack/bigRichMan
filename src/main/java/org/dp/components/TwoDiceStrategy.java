...
public class TwoDiceStrategy extends Component implements DiceStrategy {
    ...
    private AnimationTimeHelper animationTimeHelper = null;    //记录动画进度，现在选取两个骰子动画进度一样的方式
    private double lastProgress = 0;     //记录？？
    private int status = 0;
    private Vector2i startPos;   //起点

    //构造函数
    public TwoDiceStrategy(Vector2i p) {
        super(p,new Vector2i(128,64));   //这里初始化位置及该Component组件框大小，p是传入的位置
    ...
    public void drawMe(Graphics graphics) {
        ...
        if(!isRolling){     //如果骰子不在翻滚状态，分别画两个骰子的图
    ...
    //两个骰子的情况下
    //我们要做到，玩家点击其中一个骰子，就同时进行两个骰子的投掷
    //我们肯定是得创建两个骰子变量，并用同一个isRolling来表征他们的状态
    //这样是可以做到同时止停的
    //我们必须画出两个骰子的图像，用drawMe方法
    //还要能获取用户点击骰子后触发的事件
